...
public class real_fscs {
	...
	public real_fscs(ArrayList<Dimension> dim_list,String name) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//				System.out.println(temp.list.get(0));
				candidate.add(temp);
			}
			testcase= Best_candidate(Selected, candidate);
//			for (int i=0;i<5;i++){
//			System.out.println(testcase.list.get(i)+"  ");
//			}
		}
...
//		list.add(d);
		...
		for (int i = 1; i <= times; i++) {
			...
			temp=new real_fscs(list,name).run();
